...
/**
 * Internal Phase 2A premium calculation foundation.
 */
@Service
public class PremiumPayCalculationService {
	...
	public PremiumPayCalculationResult calculate(
		...
		List<Instant> boundaries = calculationBoundaries(intervalStart, intervalEnd, zone, rules);
	...
	private PremiumPaySegment segment(
		...
		List<AppliedPremiumRule> matchingRules = rules.stream()
				.filter((rule) -> applies(rule, segmentStart, zone))
	...
	private boolean applies(RuleSnapshot rule, Instant segmentStart, ZoneId zone) {
		...
		return switch (rule.type()) {
			...
			case DAILY_OVERTIME, WEEKLY_OVERTIME -> false;
		};
	}
